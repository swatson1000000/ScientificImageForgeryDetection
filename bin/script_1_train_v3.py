...
# These were discovered from previous training runs
DEFAULT_HARD_NEGATIVE_IDS = [
]


def load_hard_negative_ids(skip_hard_negatives=False):
    """Load hard negative IDs from file, or return defaults if file doesn't exist."""
    if skip_hard_negatives:
        print("Skipping hard negatives (--no-hard-negatives flag)")
        return []
...
def collect_data(skip_hard_negatives=False):
    ...
    # Hard negatives (highest priority) - load dynamically from file or use defaults
...
def main():
    ...
    parser.add_argument('--no-hard-negatives', action='store_true',
                        help='Skip hard negatives during initial training')
    ...
    # Collect data
    forged_images, forged_masks, authentic_images, hard_negatives = collect_data(args.no_hard_negatives)
    print()
    ...
    # Skip hard_negative model if --no-hard-negatives flag is set
    if args.no_hard_negatives and 'hard_negative' in models_to_train:
        models_to_train.remove('hard_negative')
        print("Skipping hard_negative model (--no-hard-negatives flag set)")
